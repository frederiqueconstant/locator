				...
				handler(new_filtered_locations,old_filtered_locations){
					...
					if(Array.isArray(new_filtered_locations))new_filtered_locations.forEach((location, loc_index) =>{
						...
						if(this.markerCluster){
							...
							this.markerCluster.getMarkers().forEach
						...
						if(this.map_check_or_init()){
							if(!this.markerCluster){
								...
								this.markerCluster = new MarkerClusterer(this.map, this.locations.map(location=>location.marker),markerClusterOptions);//
							}
							if(1){
								...
								this.markerCluster.resetViewport(true);
							}
							//this.markerCluster.repaint()
						}
				...
				markerCluster_check_or_init(){
					if(this.map_check_or_init()){
						if(!this.markerCluster){
							...
							this.markerCluster = new MarkerClusterer(this.map,markers,markerClusterOptions/*this.filtered_locations.map(location=>location.marker)*/);//,markerClusterOptions
						}
					}else{
						return null
					}
